...
response = client.post('/api/v1/checkout/order', json={'items': [{'product_id': '123e4567-e89b-12d3-a456-426614174000', 'quantity': 1}], 'delivery_address': 'Tashkent', 'payment_method': 'Cash', 'delivery_method': 'Delivery', 'customer_notes': 'Call me'})
print("CHECKOUT RESPONSE:", response.status_code, response.json())

response = client.post('/api/v1/checkout/order', json={'items': [{'product_id': 'not-a-uuid', 'quantity': 1}], 'delivery_address': 'Tashkent', 'payment_method': 'Cash', 'delivery_method': 'Delivery', 'customer_notes': 'Call me'})
