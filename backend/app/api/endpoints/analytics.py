...
async def get_dashboard_analytics(db: AsyncSession = Depends(get_db)):
    ...
    for order in recent_orders_result.scalars().all():
        # Get user details for order (this is a simplified approach, usually we'd join)
    ...
    # 7. Monthly sales mock data (since we might not have 6 months of historical data)
    # Ideally this would group by date_trunc('month', created_at)
    monthly_sales = [
        ...
        {"month": "Iyun", "revenue": total_revenue if total_revenue > 0 else 35000000}, # Use actual if available for latest
    ]
