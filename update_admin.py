...
filepath = 'milliy_metr_admin/lib/core/providers/admin_providers.dart'

with open(filepath, 'r', encoding='utf-8') as f:
    content = f.read()

# Replace any zero-width characters (e.g., \u200b, \ufeff)
content = content.replace('\u200b', '').replace('\ufeff', '').replace('\u200c', '')

# We will just rewrite the _initialCategories manually in the python script to be safe
# because regex might be tricky for all the quotes.

# Find the start and end of _initialCategories
start_idx = content.find('static final List<Map<String, dynamic>> _initialCategories = [')
end_idx = content.find('];', start_idx) + 2

# We will just write a clean version
clean_array = '''static final List<Map<String, dynamic>> _initialCategories = [
...
new_content = content[:start_idx] + clean_array + content[end_idx:]

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(new_content)

print("Updated admin_providers.dart successfully!")
